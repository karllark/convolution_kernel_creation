...
pro conv_image,file,kernel_file,out_file,conserve_flux=conserve_flux, $
               ...
               image_scale=image_scale,fix_header=fix_header, $
...
if (file_test(file)) then begin
    ...
    if (fcb.nextend LE 1) then do_unc = 0
    if (keyword_set(do_unc)) then begin
        fits_read,fcb,image_unc,header_unc,exten_no=2
    endif
    fits_close,fcb
endif else begin
...
if (keyword_set(fix_header)) then begin
    ...
    if (hpos1 NE -1) then header[hpos1] = ''
    if (hpos2 NE -1) then header[hpos2] = ''
endif

; ensure image is a single plane
image_size = size(image)
if (image_size[0] GT 2) then image = temporary(image[*,*,0])
...
if ((not keyword_set(no_rebin)) AND ((out_pixel_scale/image_scale[0]) GE 3.0)) then begin
    ...
    image_size = size(image)
    newx = image_size[1]*(image_scale[0]/out_pixel_scale)
    newy = image_size[2]*(image_scale[0]/out_pixel_scale)
...
;    out_image = change_image_scale(image,image_scale,psf_scale)
    ...
    out_image = convolve(out_image,new_psf_image,/no_pad)
...
if (n_elements(silent) EQ 0) then print,'changing image scale from ' + strtrim(string(image_scale,format='(F8.4)'),2) + $
...
conv_fac = out_pixel_scale^2/(orig_image_scale[0]*orig_image_scale[1])
old_fluxconv = fxpar(out_header,'FLUXCONV')
if (old_fluxconv EQ 0.) then begin
    new_fluxconv = 1e6*((out_pixel_scale/3600.)*!PI/180.)^2
endif else begin
    new_fluxconv = old_fluxconv*conv_fac
endelse
sxaddpar,out_header,'FLUXCONV',new_fluxconv
sxaddpar,out_header,'ZEROPT',fxpar(out_header,'ZEROPT')*conv_fac

;print,'CD1_1 = ',fxpar(out_header,'CD1_1')
...
fits_read,out_file,test_image,test_header
;print,'CD1_1 = ',fxpar(test_header,'CD1_1')
